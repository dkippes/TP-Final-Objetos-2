...
public class Location {
	...
	public double distanceBetweenTwoLocations(Location location2) {
		...
		double va1 = Math.pow(sindLat, 2)
				+ Math.pow(sindLng, 2) * Math.cos(Math.toRadians(this.getLatitude())) * Math.cos(Math.toRadians(location2.getLatitude()));
	...
	public List<Location> getNearLocationsInDistance(List<Location> locations, double kilometers) { 
		return locations.stream()
					.filter(location -> this.distanceBetweenTwoLocations(location) <= kilometers)
					.collect(Collectors.toList());
	}

	public List<Sample> getNearSamplesInDistance(Sample sample, List<Sample> samples, double kilometers) {
		return samples.stream()
				.filter(sampleType -> sample.getActualResult().equals(sampleType.getActualResult()))
				.filter(sampleLocation -> sample.getLocation().distanceBetweenTwoLocations(sampleLocation.getLocation()) <= kilometers)
					.collect(Collectors.toList());
	}
